C
C Released version in Jan 2020, version 2.1.13
C
C
...
c        write(6,*) i,prd(i),resd(ists(i)),allins(ists(i)),pcd(i)
	if(nevalue.eq.0) then
     ...
     3 calcc1dis(base(j+1,i),i), j=1,base(1,i))
...
160   format('#====='/'# Generated by BPFIND release Oct 2020, version
     1 2.1.13'/'#=====')
...
C NNW : NO.OF HELICES CONTAINING AT LEST ONE NON-WATSON-CRICK BP
     ...
     1          allins
       CHARACTER*80 LINE,nmpass,filenm,file2,text2wr
...
c     1 bpair(i)
           write(81,162) ires1(i),base1(i),ires2(i),ires3
           ENDDO
         K=1
         ENDIF
       ENDDO
